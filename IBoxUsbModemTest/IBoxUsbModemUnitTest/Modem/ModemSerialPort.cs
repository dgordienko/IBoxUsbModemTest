...
{
    public sealed class ModemSerialPort
    {
        private static readonly Lazy<ModemSerialPort> lazy = new Lazy<ModemSerialPort>(() => new ModemSerialPort());
        public static ModemSerialPort Instance => lazy.Value;

        private readonly SerialPort serial;
        private Thread _readThread;
        private volatile bool _keepReading;

        private ModemSerialPort()
        {
        ...
        public void SendString(string message)
        {
            if (serial.IsOpen)
            {
                try
                {
                    ...
                    {
                        while (serial.BytesToRead != 0) buffer.Append(serial.ReadExisting());
